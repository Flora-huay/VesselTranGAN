...
from medical_projects.ct2cta.scripts.pix2pix2d_attention import Pix2Pix2dAttention
...
def get_opt(
    ...
    parser.add_argument('--center_in_size', type=int, default=0)
    parser.add_argument('--center_depth', type=int, default=0)
    ...
    if opt.latest_train_epoch >= 0:
        model_path = os.path.join(opt.save_path, 'full_epoch{:05}.pth'.format(opt.latest_train_epoch))
        assert os.path.exists(model_path), 'model: {} is lossed!!'.format(model_path)
        print('model: {} is exists!!'.format(model_path))
...
if __name__ == '__main__':
    train_opt = get_opt(
        dataset_root="/lfs/huayun/dataset_split_image",
        vgg16_model_path='/lfs/huayun/vgg16.pt',
    ...
    train_model = Pix2Pix2dAttention(train_opt)
